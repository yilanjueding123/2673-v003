...
INT32S xdc_read_dma_phase(INT32U *buf, INT8U ifwait);
INT32S xdc_read_cmdend_phase(void);
INT32S xdc_write_cmd_phase(INT32U lba, INT32U seccount);
INT32S xdc_write_dma_phase(INT32U *buf, INT8U ifwait);
...
INT32S xdc_read_dma_phase(INT32U *buf, INT8U ifwait)
{
    ...
    if (ifwait == 1)
    {
        sts = DrvXD_read_sector(xdc_StartSectorNum + 0, 1 , (INT32U)buf);
        xdc_StartSectorNum ++;
    }
    if (ifwait == 2)
    {
        //此時buffer addr為null，使用ifwait為0時的buffer addr
        sts = DrvXD_read_sector(xdc_StartSectorNum + 0, 1 , xdc_BufferPtr);
        xdc_StartSectorNum ++;
    }//*/
    return sts;
}
...
INT32S xdc_write_dma_phase(INT32U *buf, INT8U ifwait)
{
    ...
    if (ifwait == 1)
    {
        sts = DrvXD_write_sector(xdc_StartSectorNum + 0, 1 , (INT32U)buf);
        xdc_StartSectorNum ++;
    }
    if (ifwait == 2)
    {
        //此時buffer addr為null，使用ifwait為0時的buffer addr
        sts = DrvXD_write_sector(xdc_StartSectorNum + 0, 1 , xdc_BufferPtr);
        xdc_StartSectorNum ++;
    }//*/
