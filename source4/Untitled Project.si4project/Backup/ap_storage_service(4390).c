...
RE_DEL:
        ...
        if (bkground_del_disble == 1) /*√ˆ≥¨≠I¥∫¨Â¿…≠n∞µ™∫®∆*/
        {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // §~≠Ë¨Âßπ∞®§W§Sª°§£∞˜, πJ®Ï§p¿…, •[≥t∞ª¥˙ 1 second scan
                sys_set_timer((void *)msgQSend, (void *)StorageServiceQ, MSG_STORAGE_SERVICE_FREESIZE_CHECK, storage_freesize_timerid, 128);
            }
        }
    }
...
INT32S ap_storage_service_storage_mount(void)
{
    ...
    if (device_plug_phase == 0)
    {
        ...
        nRet = _devicemount(MINI_DVR_STORAGE_TYPE);		//ÊåÇËΩΩusbÊàñËÄÖsdÂç°
    ...
    {
        if(card_first_dete)			//first detect card, in order to create file folder, as "VIDEO" or "IMAGE".
        {
            card_space_less_flag = 0;
        ...
        if (curr_storage_id != MINI_DVR_STORAGE_TYPE)
        {
            ...
            if (sd_upgrade_file_flag == 0)
            {
                ...
                gp_strcpy((INT8S *)FileName_Path, (INT8S *)"C:\\JH_");		//ÊØîËæÉÊñá‰ª∂ÁöÑÂêçÂ≠ó
...
#if SECRECY_ENABLE
					msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_SECU_READ, NULL, NULL, MSG_PRI_NORMAL);//º”√‹º∆À„
#endif
...
extern INT32S  sensor_updown_flag;
...
/*****************************************************************************
 * ∫Ø  ˝ √˚  : get_file_final_avi_index
 * ∏∫ ‘ »À  : Zhibo
 * ¥¥Ω®»’∆⁄  : 2017ƒÍ5‘¬9»’
 * ∫Ø ˝π¶ƒ‹  : ªÒ»°◊Ó∫ÛAVIŒƒº˛µƒÀ˜“˝∫≈
 *  ‰»Î≤Œ ˝  : void  NULL
 *  ‰≥ˆ≤Œ ˝  : Œﬁ
 * ∑µ ªÿ ÷µ  : 
 * µ˜”√πÿœµ  : 
