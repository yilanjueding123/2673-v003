...
INT8S ap_video_record_func_key_active(INT32U state)
{
    ...
    if (video_record_sts & VIDEO_RECORD_BUSY)			//录像过程，停止录像的操作
    {
        led_red_on();
#if C_AUTO_DEL_FILE == CUSTOM_ON
        INT8U type = FALSE;
        ...
        else
        {
            ...
            {
                _setfattr((CHAR *) curr_file_info.file_path_addr, D_RDONLY);		//设置文件的属性
        ...
        ap_video_record_sts_set(~VIDEO_RECORD_BUSY);
        msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_TIMER_START, NULL, NULL, MSG_PRI_NORMAL);//停止录像，继续检测卡的情况
        if (state == STATE_VIDEO_RECORD)
        {
            led_type = LED_WAITING_RECORD;
        }
        ...
        video_down_flag = 0;
		msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_SENSOR_DOWN, NULL, NULL, MSG_PRI_NORMAL);		//录像过程中按键，pull down摄像头
        return STATUS_FAIL;
    }
    else			//启动录像过程
    {
...
void ap_video_record_reply_action(STOR_SERV_FILEINFO *file_info_ptr)
{
    ...
    }*/
    {
        ...
        time_interval = 3 * VIDEO_RECORD_CYCLE_TIME_INTERVAL + 112;  // cycle record time interval, when time arival, save it then begin next record 
    }
#endif
    ...
    {
        if (video_encode_start(src) != START_OK)	//开始录像
        {
            close(src.type_ID.FileHandle);
            unlink((CHAR *) curr_file_info.file_path_addr);
        ...
        else
        {
            led_type = LED_RECORD;
            msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_LED_SET, &led_type, sizeof(INT32U), MSG_PRI_NORMAL);
            DBG_PRINT("video_encode_start success\r\n");

#if C_CYCLIC_VIDEO_RECORD == CUSTOM_ON
            if(1) //(cyclic_record_timerid == 0xFF)
            {
    ...
    {
        DBG_PRINT("video failed, no free space or file open handle failed\r\n");
