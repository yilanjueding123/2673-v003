...
UART_INIT_PARA uart_init_flg;
void state_startup_init(void)
{
    DBG_PRINT("Startup state init enter\r\n");
	uart_init_flg = UART_INIT_BEGIN;
    ap_startup_init();
...
void state_startup_exit(void)
{
    DBG_PRINT("state_startup_exit\r\n");
    msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_USBD_DETECT_INIT, NULL, NULL, MSG_PRI_NORMAL);		//初始化完成以后，检测usb连接状态
    OSQPost(StateHandlingQ, (void *) STATE_VIDEO_RECORD);
	uart_init_flg = UART_INIT_FINISH;
}