...
RE_DEL:
        ...
        if (bkground_del_disble == 1) /*關閉背景砍檔要做的事*/
        {
...
INT32S ap_storage_service_storage_mount(void)
{
    ...
    if (device_plug_phase == 0)
    {
        ...
        nRet = _devicemount(MINI_DVR_STORAGE_TYPE);		//?蝸usb?d??
    ...
    else
    {
        ...
        if (curr_storage_id != MINI_DVR_STORAGE_TYPE)
        {
            ...
            if (sd_upgrade_file_flag == 0)
            {
                ...
                gp_strcpy((INT8S *)FileName_Path, (INT8S *)"C:\\JH_");		//瘥??辣??摮?
...
#if SECRECY_ENABLE
					msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_SECU_READ, NULL, NULL, MSG_PRI_NORMAL);//樓躇數呾
#endif
...
/*****************************************************************************
 * 滲 杅 靡  : get_file_final_avi_index
 * 蛹 孮   : Zhibo
 * 斐膘  : 2017爛5堎9
 * 滲杅髡夔  : 鳳郔綴AVI恅璃腔坰竘瘍
 * 怀統杅  : void  NULL
