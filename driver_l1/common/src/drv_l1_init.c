...
void drv_l1_init(void)
{
    ...
    // Then initiate those hardware modules that are bus masters, like: DMA, JPEG, scaler, PPU(物理运算处理器), SPU, TFT
    ...
    exception_table_init();				// Initiate CPU exception handler(异常处理程序)
...
#if _DRV_L1_TFT==1
    tft_init();							// Initiate TFT controller(薄膜晶体管的缩写)
#endif
