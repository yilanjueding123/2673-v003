...
INT32S timer_freq_setup(INT32U timer_id, INT32U freq_hz, INT32U times_counter, void (*TimerIsrFunction)(void))
{
    if (times_counter == 0)
    {
        if ( timer_freq_normal_setup(timer_id, freq_hz, TimerIsrFunction) == TIMER_FAIL)
        {
            return TIMER_FAIL;
        }
    }
    ...
    {
        if ( timer_freq_times_setup(timer_id, freq_hz, times_counter, TimerIsrFunction) == TIMER_FAIL)
        {
            return TIMER_FAIL;
        }
    }
...
INT32S tiny_counter_get(void)   /* each tiny counter == 2.66us*/
{
    ...
    {
        ret_counter = (ret_counter * 78 / 96);
    }
    else if (MCLK == 63)
    {
        ret_counter = (ret_counter * 63 / 96);
    }
...
INT32S timer_msec_setup(INT32U timer_id, INT32U msec, INT32U times_counter, void (*TimerIsrFunction)(void))
{
    if (times_counter == 0)
    {
        if ( timer_msec_normal_setup(timer_id, msec, TimerIsrFunction) == TIMER_FAIL)
        {
            return TIMER_FAIL;
        }
    }
    ...
    {
        if ( timer_msec_times_setup(timer_id, msec, times_counter, TimerIsrFunction) == TIMER_FAIL)
        {
            return TIMER_FAIL;
        }
    }
...
INT32S time_base_normal_setup(INT32U time_base_id, TMBXS_ENUM tmbxs_enum_hz, void (*TimerIsrFunction)(void))
{
    ...
    if (time_base_id >= TIME_BASE_ID_MAX)
    {
        return TIME_BASE_FAIL;
    } /* Timer id  =  {Timer A, Timer B , Timer C, Timer D} which included in Timer1 reg.*/
